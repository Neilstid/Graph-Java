...
public class DFSDisplayController {
    ...
    public void initInterface(List<Vertex> vertex, List<Edge> edge, boolean isOrient, Color usedVertex, DFS _dfs) {
    	...
    	NumberOfStep = this.dfsAlgo.getVertexOrder().size() - 1;
    ...
    private void ChangeStep(int step) {	
    	...
    	if(!this.isOriented) {
        	for(int s = 1; s <= step; s++) {
        		Vertex vAlgo = this.convertVertex(this.dfsAlgo.getVertexOrder().get(s));
        		for(Edge e : vAlgo.linkToMe) {
        			if(e.getSecondVertex(vAlgo).equals(this.convertVertex(this.dfsAlgo.getPreviousVertex().get(this.dfsAlgo.getVertexOrder().get(s))))) {
        	...
        	for(int s = 1; s <= step; s++) {
        		Vertex vAlgo = this.convertVertex(this.dfsAlgo.getVertexOrder().get(s));
        		for(Edge e : vAlgo.linkToMe) {
        			if(e.getV2().equals(vAlgo) && e.getSecondVertex(vAlgo).equals(this.convertVertex(this.dfsAlgo.getPreviousVertex().get(this.dfsAlgo.getVertexOrder().get(s))))) {
        				e.setColor(ColVertex);
        			}else {
        				e.toBack();
        			}
